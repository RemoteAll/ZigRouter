...
- 性能：零分配，低延迟，符合 Zig 高性能定位，尽可能的达到最优性能。
- 跨平台：确保代码在所有支持的平台上均能正常运行，避免使用平台特定的API或特性，如果必须使用，需提供清晰的条件编译说明。
...
- 要优先考虑性能影响，尤其是在热点路径中，避免引入高开销的操作，如频繁的内存分配、阻塞调用等。必要时可添加性能注释说明预期影响。
- 可以优先使用已有的内部工具库和模块，目前已知的内部工具库为zzig，要避免重复造轮子。如需新增工具或模块，需评估其通用性和复用价值，并添加完善的文档说明。
- 日志相关代码应使用已有的日志模块，确保日志输出的一致性和可配置性。避免直接使用标准输出或错误输出进行日志记录，且日志级别和格式应符合项目规范。控制台输出应使用 Console 模块，确保跨平台兼容和性能优化。
- 不要为了简化而妥协实际需求。
